...
try:
    # Block only the fixture read AFTER the worker has durably claimed its job.
    ...
    for _ in range(120):
        ...
        if report:
            break
        time.sleep(0.1)
    assert report is not None, "recovery report missing"
    job.refresh_from_db()
    assert job.status == "succeeded" and job.attempt_count == 2
    assert JobAttempt.objects.filter(job=job, status="abandoned").count() == 1
    growth = call(
        "GET",
        f"/children/{child['id']}/growth-overview?from=2026-09-01T00:00:00Z&to=2026-09-08T00:00:00Z",
    )
    assert (
        growth["stage_status"] == "ready"
        and growth["robot_observation"]["metrics"][0]["value"] == 3
    )
    report_id = str(report.pk)
    # Real Django admin login, session cookie and CSRF, then execute actual child deletion.
    username = "http-smoke-" + uuid.uuid4().hex
    password = secrets.token_urlsafe(32)
    operator = get_user_model().objects.create_user(
        username=username, password=password, account_kind="staff", is_staff=True
    )
    operator.groups.add(Group.objects.get(name="technical"))
    try:
        staff_jar = http.cookiejar.CookieJar()
        staff_opener = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(staff_jar))
        with staff_opener.open("http://127.0.0.1:8017/admin/login/") as response:
            response.read()
        csrf = next(c.value for c in staff_jar if c.name == "csrftoken")
        with staff_opener.open(
            urllib.request.Request(
                "http://127.0.0.1:8017/admin/login/",
                data=urlencode(
                    {
                        "username": username,
                        "password": password,
            ...
            assert response.url.endswith("/admin/")
        receipt = call(
            "POST",
            f"/children/{child['id']}/data-requests",
            {
                "request_id": str(uuid.uuid4()),
                "kind": "deletion",
                "reason_code": "delete_child_data",
            },
        )
        csrf = next(c.value for c in staff_jar if c.name == "csrftoken")
        req = urllib.request.Request(
            base + "/staff/data-requests/" + receipt["id"] + "/resolve",
            data=json.dumps({"action": "execute_deletion", "resolution_code": "deleted"}).encode(),
            headers={"Content-Type": "application/json", "X-CSRFToken": csrf},
        )
        with staff_opener.open(req) as response:
            resolved = json.load(response)
        assert resolved["child_id"] is None and resolved["status"] == "completed"
        assert call("GET", "/data-requests")["items"][0]["resolution_code"] == "deleted"
        assert not ReportVersion.objects.filter(pk=report_id).exists()
    finally:
        operator.delete()
    print(
        json.dumps(
            {
                "m3_http_smoke": "passed",
                "worker_sigkill_recovery": "passed",
                "attempts_before_deletion": 2,
                "stage_report_id_before_deletion": report_id,
                "staff_http_login_and_deletion": "passed",
