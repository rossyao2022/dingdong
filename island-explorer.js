...
(() => {
 ...
 function render(){return `<div class="wonder-heading v4-heading"><div><span class="wonder-eyebrow">DINGDONG EXPLORER CLUB · RIASEC</span><h1>六座小岛，<em>不止一种可能。</em></h1><p>如果能在岛上生活一段时间，你最想去哪三个？</p></div><div class="exploration-shortcuts"><a class="fingerprint-shortcut" href="#talents">八大天赋优势 <span>↗</span></a><a class="fingerprint-shortcut" href="#fingerprint">指纹小宇宙 <span>↗</span></a></div></div>
 ...
 function question(){
  ...
  bridge.dialog?.('想一想，你喜欢这样玩吗？',`<div class="dialog-body interest-question"><div class="interest-question-meta"><span class="interest-code-badge" style="--island-color:${p.color}">${p.id} · ${p.name}</span><span>第 ${state.index+1} / 9 题</span></div><div class="interest-progress" role="progressbar" aria-label="答题进度" aria-valuemin="0" aria-valuemax="9" aria-valuenow="${Object.keys(state.answers).length}"><i style="width:${Object.keys(state.answers).length/9*100}%"></i></div><div class="interest-question-scene"><img src="assets/generated/island-${p.id}-v5.webp" alt="" width="140" height="140"><h3 tabindex="-1" id="interest-question-title">${q.text}</h3></div><div class="interest-ratings" role="group" aria-label="对这个活动的喜欢程度">${['很不喜欢','不太喜欢','不确定','有点喜欢','非常喜欢'].map((label,i)=>`<button class="interest-rating ${state.answers[q.id]===i?'selected':''}" data-interest-action="answer" data-rating="${i}" aria-pressed="${state.answers[q.id]===i}"><span aria-hidden="true">${['☁','◔','◉','✿','★'][i]}</span>${label}</button>`).join('')}</div><p class="interest-question-note">回答“想不想做”，不用判断“做得好不好”。没有试过，可以选不确定。<br>${storageOK?'答案自动保存在当前浏览器，可关闭后继续。':'答案仅在页面内保留，刷新后可能丢失。'}</p><span class="interest-method-label">所选三类的兴趣情境体验 · 非 O*NET 正式量表</span></div>`,`<button class="button ghost" data-interest-action="previous" ${state.index===0?'disabled':''}>上一题</button><button class="button" data-interest-action="next" ${validRating(state.answers[q.id])?'':'disabled'}>${state.index===8?'看看我的兴趣组合':'下一题 →'}</button>`,'MY THREE ISLANDS · '+state.selected.join(' · '));
 }
 function result(){
